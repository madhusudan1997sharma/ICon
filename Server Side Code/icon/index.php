...
switch($action) 
{
	...
	case "authenticateUser":
		...
		{					
			...
			$sql = "select u.Id, u.name, u.username, (NOW()-u.authenticationTime) as authenticateTimeDifference, u.IP, 
    ...
    . "left join users u on u.Id = m.fromuid WHERE `touid` = ".$userId." AND `read` = 0 LIMIT 0, 30 ";
			...
			if ($result = $db->query($sql))			
			{
					...
					while ($row = $db->fetchObject($result))
					{
						...
						$out .= "<friend  name = '".$row->name."'  username = '".$row->username."'  status='".$status."' IP='".$row->IP."' userKey = '".$row->Id."'  port='".$row->port."'/>";
